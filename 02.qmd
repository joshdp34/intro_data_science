...
- **Recognize a few ways plots can mislead** — sometimes accidentally, sometimes on purpose. Every plot you make is a claim about the data; part of learning to build plots is learning to read them critically.
...
2. **Aesthetic mappings** (`aes()`) — a description of *which variable* controls *which visual property*: x position, y position, color, shape, size, transparency (`alpha`), and so on.
3. **A geometric object** (a `geom_*()` function) — the actual visual mark used to represent each observation: points, bars, lines, boxes, ...
...
- **Global**, inside `ggplot(aes(...))` — applies to every geom layer in the plot.
- **Local**, inside a specific `geom_*(aes(...))` — applies only to that layer.
...
**Bar charts** display the distribution of a categorical variable — for example, counting penguins by species. `geom_bar()` counts observations for you; you never need to compute the counts yourself first.
...
By default, `geom_bar()` orders a character/categorical x-axis alphabetically, which is rarely the most informative order. `forcats::fct_infreq()` reorders the categories by how often they occur, so the tallest bar comes first:
...
**Three or more variables** — once you've mapped x, y, and color/shape, you can split the plot into a grid of subplots — one per category — using `facet_wrap()`:
...
When an x-axis variable only takes a handful of distinct values (like `stress_level` on a 1–5 scale), a scatterplot can hide dozens of points stacked exactly on top of each other. `geom_jitter()` adds a small amount of random noise to each point's position, purely for visibility — it doesn't change the underlying data, only how it's drawn:
...
`stress_level` in our student survey is recorded as an integer from 1–5. If you map it to `color` directly, ggplot2 assumes it is a *continuous* measurement and draws a smooth color gradient with a colorbar legend — not five distinct colors:
...
The underlying data never changed — only how ggplot2 was told to *interpret* it. Whenever a legend looks like a continuous gradient but you expected discrete groups, this is the first thing to check.
:::
...
| **Geom** | The geometric object (`geom_point()`, `geom_bar()`, `geom_histogram()`, `geom_boxplot()`, ...) used to represent each observation. |
...
3. A classmate maps a 1–5 satisfaction rating to `color` in a scatterplot and gets a smooth blue-to-yellow gradient legend instead of five distinct colors. What happened, and how would you fix their code?
...
3. They mapped a *numeric* column directly to `color`, so ggplot2 assumed it was a continuous measurement and drew a gradient. Wrapping it in `factor()` — e.g., `aes(color = factor(rating))` — tells ggplot2 to treat each rating as a discrete category, producing five distinct colors and a discrete legend instead.

4. No — it means the bin width is too narrow relative to the natural variability in the data, so each bin is mostly capturing random noise rather than the underlying shape. Try several wider bin widths (e.g., 5, 10, 30) and compare; the "right" bin width is often a range of reasonable choices rather than a single correct number.

5. Check where the y-axis actually starts. If it doesn't start at zero (here, it starts at 240, not far below the smallest value being plotted), even a tiny absolute change can be stretched to fill the entire height of the chart, making a small change look dramatic. Ask to see the same chart with the y-axis starting at zero before drawing any conclusions.
:::
