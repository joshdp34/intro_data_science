...
Values like `TRUE`, `FALSE`, `0`, and `1` all look like they belong in a logical column, but `"cabbage"` and `"F"` (a bare letter, not `FALSE`) do not, so readxl falls back to reading the *entire* column as character rather than guessing logical and silently mangling the outliers. This is the same all-or-nothing behavior you saw with `read_csv()`'s numeric guessing back in Session 6: one inconsistent value is enough to change how every other value in the column gets interpreted, not just its own cell.
:::
